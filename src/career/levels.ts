...
export type LogicFunc = "not" | "nand" | "nor" | "and" | "or" | "xor" | "buf" | "xnor" | "xnor4" | "eq2" | "mux" | "half" | "full" | "add4" | "sr" | "dlatch" | "dff" | "schmitt" | "osc" | "div2" | "cnt4" | "sreg4" | "dlatchr" | "dffr" | "tffr" | "sreg8" | "cnt393" | "dec2" | "dec3" | "seg7" | "bcd7" | "rcdb" | "debounce" | "cmp" | "cmp2" | "timer" | "mag1" | "mag4" | "sreg595" | "addsub";
...
export const LEVELS: Level[] = [
  ...
  },
  {
    ...
    about:
      "Настоящая микросхема (Maxim, SOT-143: 1 GND, 2 IN, 3 OUT, 4 VCC): кнопка — между IN и общим, подтяжка к питанию внутри. Выход меняется, только когда вход продержался в новом состоянии 20–80 мс (по даташиту, типично 50): внутри генератор, счётчик, исключающее ИЛИ-НЕ и D-триггер. Пока вход не совпадает с выходом, счётчик считает импульсы генератора; совпал снова — счёт с нуля; досчитал — триггер берёт вход. Проверка — дребезг при нажатии и отпускании и импульс 1 мс.",
    hints: [
      "Сравните очищенный вход с выходом: пока они равны, счётчик держите в сбросе (у 74HC393 сброс — единицей). Не равны — пусть считает импульсы генератора. Сброс лучше слегка задержать RC-цепью: иначе, едва триггер взял вход, сброс гасит фронт, который его взвёл.",
      "Период генератора на триггере Шмитта ≈ 0,9·R·C. Сколько импульсов нужно насчитать, чтобы вышло 20–80 мс? Какой разряд 393-го станет единицей ровно тогда — его фронт и защёлкнет вход в D-триггер. Незанятый счётчик держите в сбросе.",
    ],
    absMax: 6,
    package: "SOT-143",
    roles: ["gnd", "in", "out", "vcc"],
  ...
  // ─── Числа: разряд компаратора → 74HC85; 74HC595; проект АЛУ ─────────────────────────────
  {
    ...
    hints: [
      "«Больше» в этом разряде — A и не B; «меньше» — не A и B. Равны — когда ни то ни другое: это ИЛИ-НЕ от них двух.",
      "Выход «больше» — «больше в этом разряде» ИЛИ («равны» И «>вх»). Так же «меньше»; а «равно» — «равны» И «=вх».",
    ],
    package: "DIP",
  ...
  },
  {
    ...
    hints: [
      "Сдвиговый регистр со сбросом уже есть — 74HC164. Хранение — восемь D-триггеров с общим тактом.",
  ...
  },
  {
    ...
    about:
      "Проект: сердце процессора — арифметическое устройство, которое по сигналу SUB складывает (SUB = 0) или вычитает (SUB = 1) два четырёхразрядных числа. Вычитание — это сложение с числом «наоборот»: A − B = A + (все биты B инвертированы) + 1. C4 — перенос: при вычитании единица значит «заёма не было», A ≥ B. Выводы — как у 74HC283, только вместо C0 — SUB, разряды с нуля: F0…F3. Такой микросхемы не выпускают (настоящие АЛУ, вроде 74LS181, — в корпусе на 24 вывода), это проект: в наборы других уровней и в песочницу он не попадает. Проверяется 40 наборами.",
    hints: [
      "Инвертировать бит «по заказу» умеет исключающее ИЛИ: B XOR 0 = B, B XOR 1 = не B.",
      "Та же SUB, что инвертирует B, даёт и «+1»: подайте её на вход переноса сумматора.",
    ],
    absMax: 7,
    package: "DIP",
    roles: ["out", "in", "in", "out", "in", "in", "in", "gnd", "out", "out", "in", "in", "out", "in", "in", "vcc"],
