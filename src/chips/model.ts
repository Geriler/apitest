...
export interface ChipModel {
  ...
  absMax?: number;
  /**
   * Пороги входа с гистерезисом (триггер Шмитта), доли питания: выше hi — единица, ниже lo — ноль,
   * между — как было. Нет — обычный вход КМОП: порог около половины питания (см. INPUT_BAND).
   */
  hyst?: [number, number];
}

/**
 * Обычный вход КМОП переключается около половины питания — резко: инвертор усиливает в десятки
 * раз. Неопределённым считается только узкая полоса вокруг середины — там оказывается висящий вход.
 */
export const INPUT_BAND: [number, number] = [0.45, 0.55];

/**
 * Сколько раз за один расчёт модели разрешено переключиться. Больше — петля из микросхем без
 * RC-цепи генерирует быстрее, чем видно на шкале расчёта (у настоящих — десятки мегагерц): выход
 * тогда «не определён» — посередине, как его и покажет медленный прибор.
 */
export const MAX_FLIPS = 12;
